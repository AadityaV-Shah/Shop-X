...
export default function Fragrance() {
    ...
    useEffect(() => {
        fetch("https://dummyjson.com/products/category/fragrances?limit=10")
            ...
            .catch((err) => {
                console.error("Failed to fetch fragrance products:", err);
                setLoading(false);
            });
    }, []);

    if (loading) return <p className="text-center p-6">Loading fragrances...</p>;

    return (
        <section className="max-w-7xl mx-auto px-6 py-16 border-b-4">
            <h2 className="text-3xl font-bold mb-8 text-center text-black">Fragrances</h2>
